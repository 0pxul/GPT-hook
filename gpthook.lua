...
local function updateHeadHighlights()
    ...
    for i = #headHighlights, 1, -1 do
        ...
        -- Remove if head is missing, highlight is not enabled, or BallSocketConstraint is now present
        if not info.Head or not info.Head.Parent or not headHighlightSettings.Enabled then
            ...
            if parentModel and parentModel:IsA("Model") and parentModel.Name == "Male" then
                if hasBallSocketConstraint(parentModel) then
                    removeHighlight = true
                end
            end
        end
...
local function updateESP()
    for _, object in pairs(Workspace:GetChildren()) do
        if object:IsA("Model") and object.Name == "Male" then
            -- If BallSocketConstraint exists, skip (i.e., remove ESP box if present)
    ...
    for i = #espBoxes, 1, -1 do
        ...
        -- Remove this espBox if model is gone, or if it INCLUDES BallSocketConstraint now
        if not box.Model or not box.Model.Parent or hasBallSocketConstraint(box.Model) then
            ...
            if box.Highlight and box.Highlight.Parent then
                box.Highlight:Destroy()
            end

            table.remove(espBoxes, i)
        else
            -- Normal espBox update
            updateESPBox(box)
        end
    end
end
